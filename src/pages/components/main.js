  ...
  const handleUploadAndParse = async () => {
    ...
    try {
      ...
      if (parseResponse.ok) {
        ...
        setSurveyData(data.responses); // Assuming 'responses' is the key in the response
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `${projectName}.xlsx`; // Ensure the correct file extension
        ...
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <Box display="flex" flexDirection="row" height="91.5vh" gap={3} p={3}>
              <Paper elevation={3} sx={{  padding: '1.5rem', minWidth: '300px', marginRight: '0rem', height: '100%'}}>
                <Box sx={{ mt: 0, mb: 1 }}>
                  <Typography variant="h6" sx={{fontWeight:'bold'}}>프로젝트명, % 입력</Typography>
                  <TextField
                    id="outlined-basic"
                    label="프로젝트 명"
                    variant="outlined"
                ...
                <Divider />
                <Box sx={{ mt: 1, mb: 1 }}>
                  <Typography variant="h6" sx={{fontWeight:'bold'}}>설문 파일 업로드, 범위지정</Typography>
                ...
                <Divider />
                <Box sx={{ mt: 1, mb: 1 }}>
                  <Typography variant="h6" sx={{fontWeight:'bold'}}>CATI 데이터 업로드</Typography>
                  <div style={{ display: "flex", flexDirection: 'column' }}>
                    <Button variant="contained" component="label" color="primary">
                      EXCEL 파일 선택
                ...
                <Divider />
                <Box sx={{ mt: 1, mb: 1 }}>
                  <Typography variant="h6" sx={{fontWeight:'bold'}}>음성 파일 업로드</Typography>
                  <div style={{ display: "flex", flexDirection: 'column' }}>
                    <Button variant="contained" component="label" color="primary">
                      ZIP 파일 선택
                ...
                <Divider />
                <Box sx={{ mt: 1 }}>
                  <Typography variant="h6" sx={{fontWeight:'bold'}}>파일 업로드 및 파싱</Typography>
                  <Button variant="contained" color="primary" sx={{ mt: 1, mb: 1 }} onClick={handleUploadAndParse}>
                    업로드 및 파싱
                  </Button>
                </Box>
                <Divider />
                <Box sx={{ mt: 1 }}>
                  <Typography variant="h6" sx={{fontWeight:'bold'}}>파일 다운로드</Typography>
                  <Button variant="contained" color="primary" sx={{ mt: 1, mb: 1 }} onClick={handleDownload}>
                    다운로드
                  </Button>
                </Box>
              </Paper>

              <Paper elevation={3} sx={{ flex: 3, padding: '1.5rem', height: '100%', flexGrow: 1 }}>
